...
def obtener_resumen_financiero_total():
    ...
    res_abonos = supabase.table("abonos").select("monto").execute()
    ...
    total_recaudado_abonos = sum(float(a["monto"]) for a in res_abonos.data)
    
    # --- 2. SECCIÓN PERSONAL (INGRESOS Y GASTOS PERSONALES) ---
    res_personales = supabase.table("transacciones_personales").select("tipo, monto").execute()
